...
class AllTopics extends Component {
  ...
  render() {
    ...
    const renderTopics = currentTopics.map((topic) => (
      <div key={topic.id}>
        {topic.editing ? (
          <EditComponent topic={topic} key={topic.id} />
        ) : (
    ...
    return (
      <div>
        ...
        {topicCount >= 10 ? (
          <p>
            Showing {topicsPerPage} of {topicCount} topics
          </p>
        ) : (
          <p>Showing {topicCount} topics</p>
        )}
        <Grid container>
          <Grid item xs={12}>
            {renderTopics}
        ...
        {topicCount === 0 ? (
          <div>
            ...
            <p>
              Click <Link to="/submit">here</Link> to create a new topic.
            </p>
          </div>
        ) : (
          ""
